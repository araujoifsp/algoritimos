...
const int RUN = 32; // Tamanho do run (bloco) no Insertion Sort


//Função principal Timsort
void timSort(int *arr, int n) {
    // Divide o array em runs e ordena cada run com Insertion Sort
    for (int i = 0; i < n; i += RUN) {
        insertionSort_tim(arr, i, (i + RUN - 1) < (n - 1) ? (i + RUN - 1) : (n - 1));
    }

    // Merge dos runs usando o Merge Sort
